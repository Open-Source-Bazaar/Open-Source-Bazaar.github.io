...
type AgendaToneClass = 'formation' | 'enrollment' | 'competition' | 'break' | 'evaluation';
...
const HackathonDetail: FC<HackathonDetailProps> = observer(({ activity, hackathon }) => {
  ...
  return (
    <>
          ...
          <div className={styles.heroInner}>
            <div className={styles.heroContent}>
              ...
              <h1 className={styles.title}>
                <span className={styles.heroTitlePrimary}>{name as string}</span>
              ...
              <nav className={styles.heroActions} aria-label={t('hackathon_action_hub')}>
                {primaryForm ? (
                  <a
            ...
            <div className={styles.heroVisual}>
              <div className={styles.heroVisualCard}>
                <div className={styles.heroVisualHead}>
                  <span className={styles.visualKicker}>{t('event_info')}</span>
                  <span className={styles.visualChip}>{t('hackathon_detail')}</span>
                </div>
                ...
                <div className={styles.heroVisualFoot}>
                  ...
                  <p className={styles.heroVisualCopy}>{eventRange || (summary as string)}</p>
                </div>
              </div>

              {primaryForm && (
                <div className={`${styles.heroFloatingCard} ${styles.heroFloatingCardTop}`}>
                  <span className={styles.floatingLabel}>{t(primaryForm.meta.eyebrow)}</span>
              ...
              {agendaPreview[0] && (
                <div className={`${styles.heroFloatingCard} ${styles.heroFloatingCardBottom}`}>
                  <span className={styles.floatingLabel}>{t('hackathon_agenda_preview')}</span>
                  <strong>{agendaPreview[0].name as string}</strong>
                  <p>{formatPeriod(agendaPreview[0].startedAt, agendaPreview[0].endedAt)}</p>
                </div>
              )}
            </div>
          </div>
      ...
      <section className={styles.section}>
        <Container>
          ...
          <div className={styles.themePanel}>
            <div className={styles.themeText}>{(activityType as string) || t('hackathon')}</div>
      ...
      {formGroups[0] && (
        <section id="entry-hub" className={`${styles.section} ${styles.registerSection}`}>
          <Container>
            <div className={styles.registerWrap}>
              <article className={styles.registerCard}>
                <div className={styles.registerCardInner}>
                  <p className={styles.regEyebrow}>{t('hackathon_action_hub')}</p>
                  ...
                  <nav className={styles.regActions} aria-label={t('hackathon_entry_flow')}>
                    {primaryForm && (
                      <a
                        className={styles.actionButton}
                        href={primaryForm.list[0].shared_url}
              ...
              <div className={styles.entryHub}>
                <header className={styles.entryHubHead}>
                  <p className={styles.entryEyebrow}>{t('hackathon_entry_flow')}</p>
                  <h3 className={styles.entryTitle}>{t('hackathon_action_hub')}</h3>
                  <p className={styles.entryCopy}>{t('hackathon_entry_flow_description')}</p>
                </header>
                    ...
                    <Col as="li" key={key} md={6}>
                      <article className={styles.entryCard}>
                        ...
                        <div className={styles.entryMetaRow}>
                          <span className={styles.entryMeta}>{list.length}</span>
                          <span className={styles.entryMeta}>{t(meta.eyebrow)}</span>
                        </div>

                        <nav className={styles.entryLinks} aria-label={t(meta.title)}>
        ...
        <section id="schedule" className={styles.section}>
          <Container>
            ...
            <div className={styles.scheduleDays}>
              {agendaItems.map(({ id, name, type, summary, startedAt, endedAt }, index) => {
                const toneClass = agendaToneClassOf(type, index);

                return (
                  <article key={id} className={`${styles.dayCard} ${styles[toneClass]}`}>
                    <div className={styles.dayCardHead}>
                      <span className={styles.dayNo}>
                        PHASE {String(index + 1).padStart(2, '0')}
                      </span>
                      <time className={styles.dayDate} dateTime={startedAt as string}>
                    ...
                    <dl className={styles.dayAgenda}>
                      <div>
                        ...
                        <dd className={styles.agendaCopy}>
                          ...
                          <span>{(summary as string) || eventRange || locationText}</span>
                        </dd>
                      </div>

                      <div>
                        <dt className={styles.timePill}>{t('start_time')}</dt>
                        <dd className={styles.agendaCopy}>
            ...
            {prizes[0] && (
              <>
                <header className={styles.sectionHeader}>
                  ...
                  <p className={styles.sectionSubtitle}>{t('hackathon_prizes')}</p>
                  <div className={styles.accentLine} />
                </header>
                    ...
                    ({ id, name, image, summary, level, sponsor, price, amount }, index) => (
                      <article key={id} className={styles.badgeTile}>
                        ...
                        <div className={styles.badgeTileBody}>
                          ...
                          <h3 className={styles.badgeTileTitle}>{name as string}</h3>
                          <p className={styles.badgeTileCopy}>
                            {(summary as string) ||
                              previewText([sponsor, price, amount], t('prizes'))}
                          </p>
            ...
            {organizations[0] && (
              <div className={styles.supportCard}>
                <div className={styles.supportCopy}>
                  <p className={styles.supportEyebrow}>{t('organizations')}</p>
                  ...
                  <p className={styles.supportDescription}>{summary as string}</p>
                </div>

                <nav className={styles.partnerGrid} aria-label={t('organizations')}>
                  {organizations.map(({ id, name, link, logo }) => {
                    const imageNode = (
                      <LarkImage src={logo} alt={name as string} className={styles.partnerLogo} />
                    );
                    ...
                    return link ? (
                      ...
                      <div key={id} className={styles.partnerLink} title={name as string}>
                        {imageNode}
                      </div>
                    );
                  })}
                      ...
                      <Col key={id} md={6} xl={4}>
                        <article className={styles.resourceCard}>
                          <div className={styles.resourceHead}>
                            <h3 className={styles.resourceTitle}>{name as string}</h3>
                          </div>
                          <p className={styles.resourceDescription}>{summary as string}</p>
                          ...
                          <nav className={styles.resourceLinks} aria-label={name as string}>
                            {sourceLink && (
                              <a
                                className={styles.entryLink}
              ...
              <>
                ...
                <Row className="g-3">
                  {projects.map(({ id, name, score, summary, createdBy, members }) => {
                    ...
                    const memberNames = (members as string[] | undefined)?.join(', ') || '—';
                        ...
                        <article className={styles.projectCard}>
                          <div className={styles.projectHead}>
                            <h3 className={styles.projectTitle}>
                              <Link href={`${ActivityModel.getLink(activity)}/team/${id}`}>
                                {name as string}
                              </Link>
                            </h3>
                            <div className={styles.scoreCircle}>{scoreText}</div>
                          </div>

                          <p className={styles.projectSummary}>{summary as string}</p>
                          ...
                          <dl className={styles.projectMeta}>
                            <div>
                              ...
                              <dd>
                                {creator?.email ? (
                                  <a href={`mailto:${creator.email}`}>{creator.name}</a>
                                ) : (
                                  creator?.name || '—'
                                )}
                              </dd>
                ...
                const content = (
                  <>
                    <LarkImage
                      ...
                      title={name as string}
                    />
                    <span className={styles.participantName}>{name as string}</span>
                  </>
                );
