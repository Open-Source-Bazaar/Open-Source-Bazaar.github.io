...
function firstParamOf(searchParams: URLSearchParams, name: string) {
  const values = searchParams.getAll(name);

  return values.length > 0 ? values[0] : undefined;
}

function applyFilters(data: IndexFundSnapshot[], searchParams: URLSearchParams) {
  const parsedCategory = firstParamOf(searchParams, 'category');
  const parsedRisk = firstParamOf(searchParams, 'riskLevel');
  const parsedLimit = Number(firstParamOf(searchParams, 'limit'));
...
export const GET = safeRoute(async request => {
  ...
  const filtered = applyFilters(snapshots, request.nextUrl.searchParams);
  const cached =
    cache != null && Date.now() - cache.timestamp < CACHE_TTL_MS && filtered.length === snapshots.length;
